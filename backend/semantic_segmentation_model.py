...
class Model:
    """Class to manage model creation and its processing function
    """

    def __init__(self, name):
        self.exec_net = None
        self.net = None
        self.input_layer_name: str = None
    ...
    def preprocess_image(self, image: np.array) -> np.array:
        """Process the image before feeding it to the network

        Args:
            image (np.array): input image

        Returns:
            processed_image (np.array): processed image as per openvino requirements
        """
    ...
    def predict(self, image: np.array) -> np.array:
        """Infer the image by the model

        Args:
            image (_type_): image not processed

        Returns:
            segmentation mask (np.array): mask already postprocessed (argmax applyed by openvino)
        """
        original_y, original_x = image.shape[:-1]
        preprocessed_image = self.preprocess_image(image)
        raw_mask = self.exec_net.infer(
            inputs={self.input_layer_name: preprocessed_image}
